...
const TemperatureDetails = () => {
    ...
    const historyData = readings.map((reading, index) => ({
        ...
        status: reading.temperature > 20 && reading.temperature < 30 ? 'Good' : 'Fair'
    }));
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">Temperature Details</h1>
                    ...
                    <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={graphData}>
                                ...
                                <YAxis label={{ value: '°C', angle: -90, position: 'insideLeft', fill: '#9ca3af' }} domain={['dataMin - 1', 'dataMax + 1']} stroke="#9ca3af" />
                                <Tooltip
                                    contentStyle={{ backgroundColor: '#fff', borderColor: '#e5e7eb', borderRadius: '0.5rem' }}
                                    itemStyle={{ color: '#1f2937' }}
                                />
                                <Legend />
                                <Line type="monotone" dataKey="temperature" stroke="#ef4444" strokeWidth={2} activeDot={{ r: 8 }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Value (°C)</th>
